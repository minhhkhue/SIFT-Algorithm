...
img0 = cv2.imread("datasets/face0.jpg")
...
while cap.isOpened():
    ...
    if len(good_matches) >= 10:
        matched_keypoints_0 = np.float32([keypoints_0[m.queryIdx].pt for m in good_matches]).reshape(-1, 1, 2)
        matched_keypoints_cam = np.float32([keypoints_cam[m.trainIdx].pt for m in good_matches]).reshape(-1, 1, 2)
    ...
    final = cv2.drawMatches(gray0, keypoints_0, gray_frame, keypoints_cam, good_matches[:50],None,matchColor=(0, 255, 0),singlePointColor=(255, 0, 0),flags=0,)
    result = cv2.resize(final, (1380, 720))
    # Display the result
